...
import com.mecong.tenderalarm.databinding.FragmentLocalMediaBinding
...
class LocalFilesMediaFragment : Fragment(), FileItemClickListener, PlaylistItemClickListener {
  ...
  override fun onCreate(savedInstanceState: Bundle?) {
    ...
    playlistViewAdapter = PlaylistViewAdapter(this.requireContext(), listOfPlaylists, this)

    mediaItemViewAdapter =
      MediaItemViewAdapter(this.requireContext(), 0, emptyList(), this, false)
  ...
  override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
    ...
    val sqLiteDBHelper = sqLiteDBHelper(this.requireContext())!!
    binding.mediaListView.layoutManager = LinearLayoutManager(this.requireContext())
  ...
  fun onPlayFileChanged(playList: SleepAssistantPlayList) {
    if (currentPlaylistID == playList.playListId) {
      binding.mediaListView.scrollToPosition(playList.index)
  ...
  private fun setMode(sqLiteDBHelper: SQLiteDBHelper) {
    var trackPosition = -1
    binding.mediaListView.adapter = when (currentPlaylistID) {
      -1L -> {
        binding.buttonAdd.setOnClickListener { addPlaylist() }
        ...
        binding.buttonAdd.setOnClickListener { performNewFileSearch() }
    ...
    binding.fragmentTitle.text = currentPlaylistTitle
    binding.mediaListView.scrollToPosition(trackPosition)
  }
  ...
  private fun addPlaylist() {
    ...
    textUrl.setText(requireContext().getString(R.string.playlist_def_name, playlistViewAdapter!!.itemCount + 1))
    ...
    if (requestCode == READ_REQUEST_CODE && resultCode == Activity.RESULT_OK) {
      ...
      if (resultData != null) {
        ...
        if (clipData == null) {
          ...
          if (folderName == null) {
            val realPath = RealPathUtil.getRealPath(this.requireContext(), uri)
          ...
          sqLiteDBHelper.addLocalMediaUrl(
            currentPlaylistID,
            uri.toString(),
            getFileNameFromContentProvider(uri)
          )
        } else {
  ...
  private fun renamePlaylist(newTitle: String?) {
    if (newTitle != null && mediaItemViewAdapter?.itemCount == 0) {
      ...
      binding.fragmentTitle.text = newTitle
    }
  }
