...
import com.mecong.tenderalarm.databinding.ActivityAlarmReceiverBinding
...
class AlarmReceiverActivity : FragmentActivity(), SensorEventListener {
  ...
  private var snoozedMinutes: Long = 0
  ...
  override fun onCreate(savedInstanceState: Bundle?) {
    ...
    try {
      ...
      binding.alarmInfo.text = entity!!.message
      Timber.i("Alarm activity start: $entity")
      val complexity = entity.complexity
      shakeCount = complexity * 2

      binding.taskNote.text = this.resources.getQuantityString(R.plurals.alarm_turn_off_prompt, shakeCount, shakeCount)

      binding.turnOffComponent.complexity = complexity
...
//                }
        ...
        binding.sleepTimer.text = context.getString(R.string.sleep_timer, sleepText)


        object : CountDownTimer(sleepText.time, 1000) {
          override fun onTick(millisUntilFinished: Long) {
            binding.sleepTimer.text = context.getString(R.string.sleep_timer, millisUntilFinished)
          }
      ...
      binding.btnSnooze3m.setOnClickListener(snoozeOnClickListener)
      binding.btnSnooze5m.setOnClickListener(snoozeOnClickListener)
