...
class AlarmNotifyingService : Service(), Player.Listener {
  ...
  private fun startSound(entity: AlarmEntity?) {
    ...
    var volumeIncreaseStep =
      if (amountIncreases.compareTo(0) == 0) 0.01f else (0.7f - defaultVolume) / amountIncreases
  ...
  private fun startAlarmNotification(context: Context, entity: AlarmEntity?) {
    ...
    val stopAction = PendingIntent.getService(context, 3, stopIntent, FLAG_IMMUTABLE)
    ...
    val pendingIntent = PendingIntent.getActivity(
      context, entity.fullScreenIntentCode,
      startAlarmIntent, PendingIntent.FLAG_CANCEL_CURRENT + FLAG_IMMUTABLE
    )
