  ...
  private fun calculatePositions(viewPortBounds: Rect): MutableList<Point> {
    ...
    for (i in 1..maxHorizontal) {
      for (j in 1..maxVertical) {
        val point = Point(
          i * (diameter + spaceHorizontal) - maxRadius,
...
//        }
    for (i in figures!!.indices.reversed()) {
      ...
      if (figure!!.isFixed) {
        canvas.drawCircle(
          figure.aimPoint.x.toFloat(),
          figure.aimPoint.y.toFloat(),
    ...
    for (i in figures!!.indices.reversed()) {
      ...
      if (!figure!!.isFixed) {
        ...
        canvas.drawCircle(
          figure.currentPoint.x.toFloat(),
          figure.currentPoint.y.toFloat(),
          figure.radius.toFloat(),
          draggableCirclePaints[i]!!
        )
      }
    }
  ...
  override fun onTouchEvent(event: MotionEvent): Boolean {
    ...
    if (event.action == MotionEvent.ACTION_UP) {
      ...
      } else if (event.action == MotionEvent.ACTION_MOVE && activeFigure != null && viewPortBoundsForEvent!!.contains(
          x,
          y
        )
      ) {
